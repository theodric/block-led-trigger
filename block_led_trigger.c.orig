...
static char *device_devstr[MAX_DEVICES] = { "299:0" };  // default example for NVMe
...
static void trace_rq_issue(void *ignore, struct request *rq)
{
        ...
        {
                if (MKDEV(MAJOR(device_dev[i]), MINOR(device_dev[i])) == MKDEV(rq->q->disk->major, rq->q->disk->first_minor))
                        device_matched = true;
        }
...
static int __init block_led_trigger_init(void)
{
        ...
        // Parse device strings (such as "259:0") into device major/minor numbers
